...
public class AcademyCatalogQueryRepositoryAdapter implements AcademyCatalogQueryPort {
    ...
    private List<AcademyModuleView> buildModules(String lang, List<LearningModuleJpaEntity> modules) {
        ...
        Map<String, List<String>> lessonIdsByModuleId = lessonRepository.findAll().stream()
                .sorted(Comparator.comparingInt(LearningLessonJpaEntity::getLessonOrder))
                .collect(Collectors.groupingBy(
        ...
        return lessons.stream()
                .map(l -> {
                    ...
                    return new AcademyLessonView(
                            l.getLessonId(), l.getModuleId(), t != null ? t.getTitle() : "", l.getLessonOrder(),
                            l.getXpReward(), stepViews);
                })
                .toList();
    }
