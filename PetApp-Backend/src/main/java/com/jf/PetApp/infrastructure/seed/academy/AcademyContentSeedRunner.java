...
public class AcademyContentSeedRunner implements ApplicationRunner {
    ...
    private void upsertDomain(DomainSeedDto dto) {
        AcademyDomainJpaEntity entity = domainRepository.findById(dto.domainId()).orElseGet(AcademyDomainJpaEntity::new);
        entity.setDomainId(dto.domainId());
        entity.setOrderIndex(dto.order());
        entity.setIconKey(dto.iconKey());
        domainRepository.save(entity);

        domainTranslationRepository.deleteByDomainId(dto.domainId());
        for (Map.Entry<String, LocalizedTextDto> translation : dto.translations().entrySet()) {
            ...
            t.setTitle(translation.getValue().title());
            t.setDescription(translation.getValue().description());
            domainTranslationRepository.save(t);
        }
    }

    private void upsertSchool(SchoolSeedDto dto) {
        AcademySchoolJpaEntity entity = schoolRepository.findById(dto.schoolId()).orElseGet(AcademySchoolJpaEntity::new);
        ...
        schoolTranslationRepository.deleteBySchoolId(dto.schoolId());
        for (Map.Entry<String, LocalizedTextDto> translation : dto.translations().entrySet()) {
    ...
    private void upsertModule(ModuleSeedDto dto, String schoolId) {
        LearningModuleJpaEntity entity = moduleRepository.findById(dto.moduleId()).orElseGet(LearningModuleJpaEntity::new);
        ...
        moduleTranslationRepository.deleteByModuleId(dto.moduleId());
        for (Map.Entry<String, LocalizedTextDto> translation : dto.translations().entrySet()) {
    ...
    private void upsertLesson(LessonSeedDto dto, String moduleId) {
        LearningLessonJpaEntity entity = lessonRepository.findById(dto.lessonId()).orElseGet(LearningLessonJpaEntity::new);
        ...
        lessonTranslationRepository.deleteByLessonId(dto.lessonId());
        for (Map.Entry<String, LessonTextDto> translation : dto.translations().entrySet()) {
    ...
    private void reconcileSteps(LessonSeedDto lesson) {
        for (AcademyLessonStepJpaEntity existing : stepRepository.findByLessonIdOrderByStepOrderAsc(lesson.lessonId())) {
        ...
        for (StepSeedDto stepDto : lesson.steps()) {
            ...
            step.setFraming(stepDto.framing() == null ? null : stepDto.framing().toUpperCase(Locale.ROOT));
            step.setCorrectOptionIndex(stepDto.correctIndex());
            step = stepRepository.save(step);

            for (Map.Entry<String, StepTextDto> translation : stepDto.translations().entrySet()) {
    ...
    private void reconcileSchoolPrerequisites(String schoolId, List<String> prerequisites) {
        schoolPrerequisiteRepository.deleteBySchoolId(schoolId);
        for (String prerequisiteId : prerequisites) {
            AcademySchoolPrerequisiteJpaEntity e = new AcademySchoolPrerequisiteJpaEntity();
    ...
    private void reconcileModulePrerequisites(String moduleId, List<String> prerequisites) {
        modulePrerequisiteRepository.deleteByModuleId(moduleId);
        for (String prerequisiteId : prerequisites) {
            AcademyModulePrerequisiteJpaEntity e = new AcademyModulePrerequisiteJpaEntity();
