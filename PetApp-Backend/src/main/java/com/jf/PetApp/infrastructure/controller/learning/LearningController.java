...
public class LearningController {
    ...
    public ResponseEntity<LessonCompletionResponseDTO> completeLesson(@PathVariable String lessonId) {
        String email = SecurityUtils.getCurrentUserEmail();
        LessonCompletionResult result = completeLessonUseCase.execute(email, lessonId);
    ...
    public record LearningProgressResponseDTO(
        ...
        static LearningProgressResponseDTO from(LearningProgressResult r) {
            return new LearningProgressResponseDTO(
                    r.completedLessonIds(), r.completedModuleIds(), r.totalXp(), r.level(), r.xpIntoLevel(), r.xpForNextLevel());
        }
    }
}
