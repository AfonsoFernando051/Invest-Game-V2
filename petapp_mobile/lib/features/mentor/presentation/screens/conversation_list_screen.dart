...
class _ConversationListScreenState extends State<ConversationListScreen> {
  ...
  Future<void> _rename(ConversationSummary conversation) async {
    ...
    if (newTitle != null && newTitle.isNotEmpty && newTitle != conversation.title) {
      await _controller.rename(conversation.id, newTitle);
    }
  }
  ...
  Future<void> _confirmDelete(ConversationSummary conversation) async {
    ...
    if (confirmed == true) {
      ...
      await _controller.delete(conversation.id);
    }
  }
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.chat_bubble_outline, size: 48, color: tokens.textSecondary),
            const SizedBox(height: 16),
            Text(
              Translator.translate(AppStrings.mentorNoConversationsTitle),
              textAlign: TextAlign.center,
              style: TextStyle(color: tokens.textPrimary, fontSize: 16, fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 6),
            Text(
              Translator.translate(AppStrings.mentorNoConversationsSubtitle),
              textAlign: TextAlign.center,
