...
void main() {
  ...
  group('loadConversation / listConversations / renameConversation / deleteConversation', () {
    test('loadConversation delegates to the data source', () async {
      final messages = [ChatMessage(id: '1', role: ChatRole.user, text: 'Oi', timestamp: DateTime(2024, 1, 1))];
      when(() => mockDataSource.getConversationMessages(7)).thenAnswer((_) async => messages);
    ...
    test('listConversations delegates to the data source', () async {
      ...
      when(() => mockDataSource.listConversations()).thenAnswer((_) async => summaries);

      expect(await repository.listConversations(), summaries);
    });

    test('renameConversation delegates to the data source', () async {
      when(() => mockDataSource.renameConversation(1, 'Novo título')).thenAnswer((_) async {});

      await repository.renameConversation(1, 'Novo título');

      verify(() => mockDataSource.renameConversation(1, 'Novo título')).called(1);
    });

    test('deleteConversation delegates to the data source', () async {
      when(() => mockDataSource.deleteConversation(1)).thenAnswer((_) async {});
  ...
  group('sendMessage', () {
    test('sends the message with the user\'s saved goal/horizon and current language as context', () async {
      ...
      when(() => mockDataSource.sendMessage(
          ...
          )).thenAnswer((_) async => const MentorChatResult(reply: 'reply', conversationId: 1));
    ...
    test('passes the given conversationId through to the data source', () async {
      when(() => mockDataSource.sendMessage(
            message: any(named: 'message'),
            conversationId: any(named: 'conversationId'),
            context: any(named: 'context'),
          )).thenAnswer((_) async => const MentorChatResult(reply: 'reply', conversationId: 42));
    ...
    test('returns the result from the data source', () async {
      when(() => mockDataSource.sendMessage(
            ...
            conversationId: any(named: 'conversationId'),
            context: any(named: 'context'),
          )).thenAnswer((_) async => const MentorChatResult(reply: 'Claro, posso ajudar!', conversationId: 5, title: 'Dúvida'));
